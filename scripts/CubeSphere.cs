...
public partial class CubeSphere : MeshInstance3D
{
    [Export]
    ...
    public override void _Ready()
    {
        ...
        var arrMesh = surfaceTool.Commit();

        ResourceSaver.Save(arrMesh, "res://CubeSphere.tres", ResourceSaver.SaverFlags.Compress);
    }
    ...
    private SurfaceTool GenerateMesh(SurfaceTool surfaceTool)
    {
        for (int i = 0; i < JobLength; i++)
        {
            ...
            for (int v = 1; v <= Resolution; v++, vi += 4, ti += 2)
            {
                ...
                surfaceTool.AddVertex(pC);
                surfaceTool.SetUV(new Vector2(0f, 1f));
                surfaceTool.AddVertex(pD);
