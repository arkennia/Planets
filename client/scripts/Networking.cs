...
//         SolarSystemNode system = new();
//         // system.Generate(new Guid(guid));
//         system.Name = system.Guid.ToString();
//         GetTree().Root.GetNode<Node>("/root/Main/Game/World").AddChild(system);
//         GameManager.AddSystem(system);
//         system.Generate(new Guid(guid));
//     }
...
//         Sun sun = new(radius: radius, guid: new Guid(guid));

//         SunNode sunNode = new()
//         {
//             Sun = sun,
//             Name = sun.Guid.ToString()
//         };
//         GameManager.AddSun(sunNode, new Guid(systemGuid));
//     }

//     [Rpc(MultiplayerApi.RpcMode.Authority, TransferMode = MultiplayerPeer.TransferModeEnum.Reliable)]
//     private void GetPlanets(byte[] id, int seed, Array<double> heights, byte[] systemGuid)
//     {
//         GD.Print("Method called by: " + Multiplayer.GetRemoteSenderId());
//         if (Multiplayer.GetRemoteSenderId() != 1) return;
...
//             GameManager.AddPlanet(planet, new Guid(systemGuid));
//             EmitSignal(SignalName.PlanetLoaded, planet);
//         }
//         else
//         {
//             EmitSignal(SignalName.SyncingFinished);
//             GD.Print($"Number of Planets received:  {GameManager.Systems.Count}");
